 ...
 * **Full results set** (coming soon): if you need to access all the raw results (i.e all S-PrediXcan and fastENLOC results across all tissues, TORUS fine-mapping results, etc), you can download them from this [UChicago Box shared folder](https://uchicago.box.com/s/i6vocl9pq59gydzpefidexodyuhegmde). The subfolder `rapid_gwas_project` contains all results for 4,049 traits from the [Rapid GWAS Project](http://www.nealelab.is/uk-biobank), and the `gtex_gwas` subfolder contains 42 extra traits from the [GTEx GWAS manuscript](https://doi.org/10.1101/814350).
