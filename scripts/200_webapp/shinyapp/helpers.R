...
get_results_from_data_db_ <- function(input, table_sme, table_g) {
  CLAUSE <- "
SELECT 
  pheno_desc as phenotype, gene_name, pheno_source as type, band, gene, rcp as colocalization, pvalue, n as n_tissues, n_indep, t_i_best as best_tissue, p_i_best as best_tissue_p
FROM smultixcan
WHERE 1=1 
" %>% glue::glue()
